...
export default function HeroSequence() {
  ...
  useGSAP(() => {
    ...
    function animateIn(index: number) {
      ...
      // Update the right-edge navigation dots
      gsap.to(".nav-dot", { backgroundColor: "transparent", duration: 0.3 });
      gsap.to(`.nav-dot-${index}`, { backgroundColor: "rgba(0,0,0,0.55)", duration: 0.3 });
    }

    // ── GSAP NATIVE PIN & SNAP ───────────────────────────────────────────────
    // This perfectly calculates the mobile viewport so you never see black!
    ScrollTrigger.create({
      trigger: containerRef.current,
      start:   "top top",
      end:     "+=300%", // 3 extra scrolls to get through the 4 flavors
      pin:     true,     // GSAP's magic bullet for pinning
      snap: {
        snapTo:   1 / 3, // Snaps perfectly to exactly one flavor at a time
        duration: { min: 0.45, max: 0.85 },
        delay:    0.05,
        ease:     "power2.inOut",
      },
      onUpdate: (self) => {
        // Reads exactly how far down you scrolled and triggers the right flavor
  ...
  return (
    <div
      ...
      style={{
        position: "relative",
        width:    "100%",
        height:   "100vh", // Clean h-screen setup, no more 400vh container!
        overflow: "hidden",
      }}
    >
      {/* Background */}
